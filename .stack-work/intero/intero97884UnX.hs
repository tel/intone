...
import qualified Music.Intonation.Pitch as P
...
standardPitch :: Note -> P.Pitch
standardPitch n = P.a440 .-^ (pitchInterval (a4 .-. n))
...
pitchInterval :: Interval -> P.Interval
pitchInterval (Interval steps) = P.semitone ^* fromIntegral steps

align :: P.Pitch -> (Note, P.Interval)
align goal =
  let
    diff = goal .-. P.a440
    (sts, error) = P.approxWith P.semitone diff
  in
    (a4 .+^ (sts *^ semitone), error)
